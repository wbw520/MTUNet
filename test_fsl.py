...
def main():
    ...
    model_name = "similarity_checkpoint_ab_att1.pth"
    ...
    for i in range(len(inputs_support)):
        iters(support_name[i], model, torch.unsqueeze(inputs_support[i], dim=0), "support/pic_" + str(cls[i].item()) + "_")
    print("------------")
    for j in range(len(inputs_query)):
        iters(query_name[j], model, torch.unsqueeze(inputs_query[j], dim=0), "query/pic_" + str(cls[j].item()) + "_")


def iters(name, model, image, record_name):
    image_orl = Image.open(name[0])
    test(args, model, image_orl, image, record_name)
